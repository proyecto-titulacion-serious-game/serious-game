...
namespace UnityJapanOffice {
	...
	public class Controller : MonoBehaviour {
		...
		private void Update () {
			if(!isPlaying) {
				if(InputWrapper.IsTriggerCloseGuideUI()) {
			...
			bool toggleMenuFromController = InputWrapper.IsTriggerToggleMenuButton();

			if ((InputWrapper.IsTriggerToggleMenuPointerButton()
				&& !IsHittingToUI(InputWrapper.GetMousePosition())) ||
				toggleMenuFromController){
				if (isOpenedMenu){
					walkThrough.LockPosition(false);
					walkThrough.LockRotation(false);
					menuUI.Hide();
					...
					walkThrough.LockRotation(true);
					menuUI.Show();
					if (toggleMenuFromController){
						GamepadUIController.current.OnStartMenuFromController();
					}
					lockSceneState = false;
				}
		...
		private bool lockSceneState = false;
